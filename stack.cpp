...
{
    myStack.pushR(newItem); //push to the right side of the deque (top of stack)
}
...
T Stack<T>::pop()
{
    return myStack.popR(); //pop the right side of the deque (top of stack)
}
...
T Stack<T>::peek()
{
    return myStack.peekR(); //peek the right side of the deque (top of stack)
}
...
{
    return myStack.isEmpty(); //check if deque is empty
}
